  ...
  const float kv_scale)
{
  ...
  // unsigned long long streamId;
  // cudaStreamGetId(stream, &streamId);
  // printf("(cache_kernels.cu) vLLM performs reshape_and_cache: tokens=%d, key_stride=%d, val_stride=%d, num_heads=%d, head_size=%d \n", num_tokens, key_stride, value_stride, num_heads, num_heads);
