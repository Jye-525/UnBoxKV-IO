...
class SchedulerConfig:
    ...
    Args:
        ...
        preemption_mode: Preemption mode for the scheduler. # add by JY for test
        enable_mixed_batch: If True, the scheduler will use mixed batching approach, which 
        enable to batch decode-phase requests with prefill-phase request.
        chunk_size: The size of the chunk to be used for chunked prefill.
    """
    ...
    def __init__(
        ...
        #     else:
        #         self.chunk_size = self.max_num_batched_tokens
        ...
        self.enable_mixed_batch = enable_mixed_batch
        if preemption_mode is not None:
            self.preemption_mode = preemption_mode
        else:
            self.preemption_mode = "recompute" # default preemption mode

        self._verify_args()
        logger.info(f"[SchedulerConfig] max_num_batched_tokens: {self.max_num_batched_tokens} chunked_prefill_enabled: {self.chunked_prefill_enabled}")
    ...
    def _verify_args(self) -> None:
        ...
        if self.preemption_mode not in ["recompute", "swap"]:
            raise ValueError(
                f"Unknown preemption mode: {self.preemption_mode}. Must be "
                "either 'recompute' or 'swap'.")
        
        if self.enable_mixed_batch and self.chunked_prefill_enabled:
            raise ValueError(
                "Mixed batching and chunked prefill are currently mutually "
                "exclusive.")
