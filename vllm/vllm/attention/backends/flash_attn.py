...
class FlashAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        if prefill_meta := attn_metadata.prefill_metadata:
            # if self.layer_id == 0:
            #     print(f"FlashAttentionImpl: block_tables.numel() is 0? {prefill_meta.block_tables.numel() == 0} kv_cache is None? {kv_cache is None}", flush=True)
            # # Prompt run.
            if kv_cache is None or prefill_meta.block_tables.numel() == 0:
                ...
                attn_fun_name = "flash_attn_varlen_func"
            else:
                ...
                attn_fun_name = "PagedAttention.forward_prefix"
        ...
        if decode_meta := attn_metadata.decode_metadata:
            ...
            attn_fun_name = "PagedAttention.forward_decode"
        torch.cuda.nvtx.range_pop()
        self.attention_time = time.time() - start_attn
        # if self.layer_id == 0:
        #     print(f"FlashAttentionImpl: layer_id={self.layer_id} write_kv_time={write_kv_time * 1e3:.4f} attention_time={self.attention_time*1e3:.4f}, attn_fun_name={attn_fun_name}", flush=True)
