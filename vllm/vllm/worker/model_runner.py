...
class ModelRunner:
    ...
    def _prepare_prompt(
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            # print(f"++++++computed_block_nums: {computed_block_nums}, prefix_block_tables: {len(prefix_block_tables)}")
            ...
            logger.debug(f"prompt block_table for seq {seq_id}: len = {len(block_table)}, slot_mapping_len={len(slot_mapping)},computed_tokens = {seq_group_metadata.seq_data[seq_id].get_num_computed_tokens()}")
        ...
        # print(f"===========block_tables shape: {block_tables.shape}, block_tables.numel() is 0? {block_tables.numel() == 0}, max_prompt_block_table_len: {max_prompt_block_table_len}, prefix_block_tables_len={len(prefix_block_tables)}")
    ...
    def _prepare_decode(
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            for seq_id in seq_ids:
                ...
                logger.debug(f"decoder block_table for seq {seq_id}: len = {len(block_table)}, slot_mapping_len={len(slot_mapping)}, computed_tokens = {seq_group_metadata.seq_data[seq_id].get_num_computed_tokens()}")
    ...
    def prepare_input_tensors(
        ...
        if self.is_driver_worker:
            ...
            if not self.scheduler_config.chunked_prefill_enabled and not self.scheduler_config.enable_mixed_batch:
                assert (len(prefill_reqs) and len(decode_reqs)) == 0
    ...
    def execute_model(
        ...
        torch.cuda.nvtx.range_push("ModelRunner.prepare_input_tensors")
        ...
        # Compute the logits.
        start_cpm_logits = time.time()
        torch.cuda.nvtx.range_push("compute_logits")
        logits = self.model.compute_logits(hidden_states, sampling_metadata)
        torch.cuda.nvtx.range_pop()
        end_cpm_logits = time.time()
        ...
        logger.info(f"ModelRunner execute_model (ms): prepare_input = {(end_prepare - start_prepare) * 1e3:.4f}, fwd={(end_fwd - start_fwd) * 1e3:.4f} cmp_logits={(end_cpm_logits - start_cpm_logits) * 1e3:.4f}, sampling={(end_sample - start_sample) * 1e3:.4f}, total={(end_sample - start_prepare) * 1e3:.4f}")
