...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        # print(f"Sampler.forward(): do_penalties={do_penalties}, do_top_p_top_k={do_top_p_top_k}, do_min_p={do_min_p}", flush=True)
...
def _apply_min_tokens_penalty(
    ...
    for seq_group in sampling_metadata.seq_groups:
        ...
        # print(f"do_sample: len(sample_indices) = {len(sample_indices)}, logits_applied={logits_applied}", flush=True)
...
def _get_logprobs(
    ...
    if len(query_indices) == 0:
        ...
        # # In case of all the requests are chunked prefill requests, then no sampling
        # for seq_group in sampling_metadata.seq_groups:
    ...
    for seq_group, sample_result in zip(sampling_metadata.seq_groups,
                                        sample_results):
        # print(f"In _get_logprobs: seq_group.is_prompt = {seq_group.is_prompt}, ids = {seq_group.seq_ids}")
    ...
    # print(f"prompt_logprobs_per_seq_group = {len(prompt_logprobs_per_seq_group)}")
    return prompt_logprobs_per_seq_group, sample_logprobs_per_seq_group
...
def _build_sampler_output(
    ...
    sampler_output = []
    # print(f"In _build_sampler_output(), len(sampling_metadata.seq_groups) = {len(sampling_metadata.seq_groups)}, len(sample_results) = {len(sample_results)}, len(prompt_logprobs) = {len(prompt_logprobs)}, len(sample_logprobs) = {len(sample_logprobs)}")
    ...
    for (seq_group, sample_result, group_prompt_logprobs,
        ...
        seq_outputs = []
        # print(f"In _build_sampler_output(), seq_group={seq_group}, seq_ids = {seq_ids}, next_token_ids={next_token_ids}, parent_ids={parent_ids}, group_sample_logprobs={group_sample_logprobs}")
    ...
    # print(f"In _build_sampler_output(), sampler_output = {sampler_output}")
